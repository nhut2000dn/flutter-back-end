...
enum statusUploadImageAndAddCategory {
  sucess,
  fail,
  failUploadStorage,
  failAddFirestore
...
class _AddCategoryPageState extends State<AddCategoryPage> {
  final CategoryService _cateogryService = CategoryService();
  ...
  Future<bool> addCategoryToFirebae(
      String name, String desciption, String image) async {
    if (name == '' || desciption == '') {
      return false;
    } else {
      return await _cateogryService.addCategory(
          {'name': name, 'description': desciption, 'image': image});
    }
  }

  Future<statusUploadImageAndAddCategory> uploadImageAndAddCategoryToFirebase(
      String name, String description) async {
    statusUploadImageAndAddCategory status =
        statusUploadImageAndAddCategory.fail;
    if (_imageFile!.path != '') {
      ...
      await Future.value(uploadTask)
          .then((value) async => {
                ...
                if (await addCategoryToFirebae(
                    name, description, await value.ref.getDownloadURL()))
                  {status = statusUploadImageAndAddCategory.sucess}
                else
                  {status = statusUploadImageAndAddCategory.failAddFirestore}
              })
          .onError((error, stackTrace) => {
                debugPrint("Upload file path error ${error.toString()} "),
                status = statusUploadImageAndAddCategory.failUploadStorage
              });
    } else {
      if (await addCategoryToFirebae(name, description, '')) {
        status = statusUploadImageAndAddCategory.sucess;
      } else {
        status = statusUploadImageAndAddCategory.failAddFirestore;
      }
                ...
                child: Container(
                  ...
                  child: FlatButton(
                    onPressed: () async {
                      ...
                      statusUploadImageAndAddCategory status =
                          await uploadImageAndAddCategoryToFirebase(
                        _nameController.text,
                        _descriptionController.text,
                      );
                      if (status == statusUploadImageAndAddCategory.sucess) {
                        locator<NavigationService>().goBack();
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content: Text("Add Category Succesful!")));
                        tablesProvider.refreshFromFirebase(Tables.categories);
                      } else if (status ==
                          statusUploadImageAndAddCategory.failUploadStorage) {
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
