...
enum statusUploadImageAndAddGenre {
  sucess,
  fail,
  failUploadStorage,
  failAddFirestore
...
class _AddGenrePageState extends State<AddGenrePage> {
  ...
  Future<statusUploadImageAndAddGenre> uploadImageAndAddGenreToFirebase(
    ...
    statusUploadImageAndAddGenre status = statusUploadImageAndAddGenre.fail;
    if (_imageFile!.path != '') {
      ...
      await Future.value(uploadTask)
          .then((value) async => {
                ...
                if (await addGenreToFirebae(
                    name, description, await value.ref.getDownloadURL()))
                  {status = statusUploadImageAndAddGenre.sucess}
                else
                  {status = statusUploadImageAndAddGenre.failAddFirestore}
              })
          .onError((error, stackTrace) => {
                debugPrint("Upload file path error ${error.toString()} "),
                status = statusUploadImageAndAddGenre.failUploadStorage
              });
    } else {
      if (await addGenreToFirebae(name, description, '')) {
        status = statusUploadImageAndAddGenre.sucess;
      } else {
        status = statusUploadImageAndAddGenre.failAddFirestore;
      }
                ...
                child: Container(
                  ...
                  child: FlatButton(
                    onPressed: () async {
                      ...
                      statusUploadImageAndAddGenre status =
                          await uploadImageAndAddGenreToFirebase(
                        _nameController.text,
                        _descriptionController.text,
                      );
                      if (status == statusUploadImageAndAddGenre.sucess) {
                        locator<NavigationService>().goBack();
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content: Text("Add Genre Succesful!")));
                        tablesProvider.refreshFromFirebase(Tables.genres);
                      } else if (status ==
                          statusUploadImageAndAddGenre.failUploadStorage) {
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
