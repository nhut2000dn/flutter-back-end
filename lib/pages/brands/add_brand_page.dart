...
enum statusUploadImageAndAddBrand {
  sucess,
  fail,
  failUploadStorage,
  failAddFirestore
...
class _AddBrandPageState extends State<AddBrandPage> {
  ...
  Future<bool> addBrandToFirebae(
      String name, String desciption, String image) async {
    if (name == '' || desciption == '') {
      return false;
    } else {
      return await _brandService
          .addBrand({'name': name, 'description': desciption, 'image': image});
    }
  }

  Future<statusUploadImageAndAddBrand> uploadImageAndAddBrandToFirebase(
      String name, String description) async {
    statusUploadImageAndAddBrand status = statusUploadImageAndAddBrand.fail;
    if (_imageFile!.path != '') {
      ...
      await Future.value(uploadTask)
          .then((value) async => {
                ...
                if (await addBrandToFirebae(
                    name, description, await value.ref.getDownloadURL()))
                  {status = statusUploadImageAndAddBrand.sucess}
                else
                  {status = statusUploadImageAndAddBrand.failAddFirestore}
              })
          .onError((error, stackTrace) => {
                debugPrint("Upload file path error ${error.toString()} "),
                status = statusUploadImageAndAddBrand.failUploadStorage
              });
    } else {
      if (await addBrandToFirebae(name, description, '')) {
        status = statusUploadImageAndAddBrand.sucess;
      } else {
        status = statusUploadImageAndAddBrand.failAddFirestore;
      }
                ...
                child: Container(
                  ...
                  child: FlatButton(
                    onPressed: () async {
                      ...
                      statusUploadImageAndAddBrand status =
                          await uploadImageAndAddBrandToFirebase(
                        _nameController.text,
                        _descriptionController.text,
                      );
                      if (status == statusUploadImageAndAddBrand.sucess) {
                        locator<NavigationService>().goBack();
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content: Text("Add Brand Succesful!")));
                        tablesProvider.refreshFromFirebase(Tables.brands);
                      } else if (status ==
                          statusUploadImageAndAddBrand.failUploadStorage) {
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
