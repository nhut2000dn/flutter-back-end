...
enum statusUploadImageAndEditGenre {
  sucess,
  fail,
  failUploadStorage,
  failAddFirestore
...
class _EditGenrePageState extends State<EditGenrePage> {
  ...
  Future<statusUploadImageAndEditGenre> uploadImageAndEditGenreToFirebase(
    ...
    statusUploadImageAndEditGenre status = statusUploadImageAndEditGenre.fail;
    if (_imageFile!.path != '') {
      ...
      await Future.value(uploadTask)
          .then((value) async => {
                ...
                if (await editGenreToFirebae(
                    id, name, description, await value.ref.getDownloadURL()))
                  {status = statusUploadImageAndEditGenre.sucess}
                else
                  {status = statusUploadImageAndEditGenre.failAddFirestore}
              })
          .onError((error, stackTrace) => {
                debugPrint("Upload file path error ${error.toString()} "),
                status = statusUploadImageAndEditGenre.failUploadStorage
              });
    } else {
      if (await editGenreToFirebae(id, name, description, '')) {
        status = statusUploadImageAndEditGenre.sucess;
      } else {
        status = statusUploadImageAndEditGenre.failAddFirestore;
      }
                ...
                child: Container(
                  ...
                  child: FlatButton(
                    onPressed: () async {
                      ...
                      statusUploadImageAndEditGenre status =
                          await uploadImageAndEditGenreToFirebase(
                        widget.argument['id'],
                        _nameController.text,
                        _descriptionController.text,
                      );
                      if (status == statusUploadImageAndEditGenre.sucess) {
                        locator<NavigationService>().goBack();
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content: Text("Update Genre Succesful!")));
                        tablesProvider.refreshFromFirebase(Tables.genres);
                      } else if (status ==
                          statusUploadImageAndEditGenre.failUploadStorage) {
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
