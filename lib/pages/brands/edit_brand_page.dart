...
enum statusUploadImageAndEditBrand {
  sucess,
  fail,
  failUploadStorage,
  failAddFirestore
...
class _EditBrandPageState extends State<EditBrandPage> {
  ...
  Future<statusUploadImageAndEditBrand> uploadImageAndEditBrandToFirebase(
      String id, String name, String description) async {
    statusUploadImageAndEditBrand status = statusUploadImageAndEditBrand.fail;
    if (_imageFile!.path != '') {
      ...
      await Future.value(uploadTask)
          .then((value) async => {
                ...
                if (await editBrandToFirebae(
                    id, name, description, await value.ref.getDownloadURL()))
                  {status = statusUploadImageAndEditBrand.sucess}
                else
                  {status = statusUploadImageAndEditBrand.failAddFirestore}
              })
          .onError((error, stackTrace) => {
                debugPrint("Upload file path error ${error.toString()} "),
                status = statusUploadImageAndEditBrand.failUploadStorage
              });
    } else {
      if (await editBrandToFirebae(id, name, description, '')) {
        status = statusUploadImageAndEditBrand.sucess;
      } else {
        status = statusUploadImageAndEditBrand.failAddFirestore;
      }
                ...
                child: Container(
                  ...
                  child: FlatButton(
                    onPressed: () async {
                      ...
                      statusUploadImageAndEditBrand status =
                          await uploadImageAndEditBrandToFirebase(
                        widget.argument['id'],
                        _nameController.text,
                        _descriptionController.text,
                      );
                      if (status == statusUploadImageAndEditBrand.sucess) {
                        locator<NavigationService>().goBack();
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content: Text("Update Brand Succesful!")));
                        tablesProvider.refreshFromFirebase(Tables.brands);
                      } else if (status ==
                          statusUploadImageAndEditBrand.failUploadStorage) {
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content: Text("Upload Brand failed!")));
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content:
                                    Text("Update Brand failed! Invalid form")));
                      }
                      setState(() {
                        _isLoading = false;
                      });
