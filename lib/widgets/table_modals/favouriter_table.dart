...
import 'package:admin_dashboard/widgets/add_modals/add_user_product.dart';
...
class FavouriterTable extends StatefulWidget {
  String id;
  FavouriterTable({
    Key? key,
    required this.id,
  ...
  @override
  _FavouriterTableState createState() => _FavouriterTableState();
}

class _FavouriterTableState extends State<FavouriterTable>
    with AutomaticKeepAliveClientMixin<FavouriterTable> {
  @override
  bool get wantKeepAlive => true;

  late TextEditingController inputSearchController;
  late bool isSearch = false;
  List<Map<String, dynamic>> favouriterTableSource = <Map<String, dynamic>>[];
  List<UserModel> _users = <UserModel>[];
  List<UserModel> get users => _users;
  List<UserModel> _allUsers = <UserModel>[];
  bool isLoading = true;

  List<Map<String, dynamic>> _getUsersFavouritedData() {
  ...
  updateTable(bool check, UserModel user) {
    if (check) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Add Favouriter Succesful!")));
      setState(() {
        favouriterTableSource.add({
          "id": user.id,
          "email": user.email,
          "name": user.fullName,
          "avatar": user.avatar,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Add Favouriter failed!")));
    }
  }
  ...
  initData() async {
    ...
    var placeholderUsers =
        await UsersProductsService().getAllUsersFollowed(widget.id);
    var placeholderAllUser = await UserServices().getAllUsers();
    setState(() {
      _users = placeholderUsers;
      _allUsers = placeholderAllUser;
      favouriterTableSource.addAll(_getUsersFavouritedData());
      isLoading = false;
    });
  }
            ...
            return Row(
              ...
              children: [
                InkWell(
                  ...
                  onTap: () async {
                    bool check = await UsersProductsService()
                        .deleteUserNovel(row['user_id']);
                    if (check) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text("Delete Favouriter Succesful!"),
                        ),
                      );
                      setState(() {
                        favouriterTableSource
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text("Delete Favouriter Fail!"),
                        ),
                      );
                    }
                  },
                ...
                child: ResponsiveDatatable(
                  title: !isSearch
                      ? RaisedButton.icon(
                          ...
                          label: const Text("ADD Favouriter"))
                      : null,
                  actions: [
                    if (isSearch)
                      Expanded(
