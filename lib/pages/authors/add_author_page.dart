...
enum statusUploadImageAndAddAuthor {
  sucess,
  fail,
  failUploadStorage,
  failAddFirestore
...
class _AddAuthorPageState extends State<AddAuthorPage> {
  ...
  Future<statusUploadImageAndAddAuthor> uploadImageAndAddAuthorToFirebase(
    ...
    statusUploadImageAndAddAuthor status = statusUploadImageAndAddAuthor.fail;
    if (_imageFile!.path != '') {
      ...
      await Future.value(uploadTask)
          .then((value) async => {
                ...
                if (await addAuthorToFirebae(
                    name, description, await value.ref.getDownloadURL()))
                  {status = statusUploadImageAndAddAuthor.sucess}
                else
                  {status = statusUploadImageAndAddAuthor.failAddFirestore}
              })
          .onError((error, stackTrace) => {
                debugPrint("Upload file path error ${error.toString()} "),
                status = statusUploadImageAndAddAuthor.failUploadStorage
              });
    } else {
      if (await addAuthorToFirebae(name, description, '')) {
        status = statusUploadImageAndAddAuthor.sucess;
      } else {
        status = statusUploadImageAndAddAuthor.failAddFirestore;
      }
                ...
                child: Container(
                  ...
                  child: FlatButton(
                    onPressed: () async {
                      ...
                      statusUploadImageAndAddAuthor status =
                          await uploadImageAndAddAuthorToFirebase(
                        _nameController.text,
                        _descriptionController.text,
                      );
                      if (status == statusUploadImageAndAddAuthor.sucess) {
                        locator<NavigationService>().goBack();
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content: Text("Add Author Succesful!")));
                        tablesProvider.refreshFromFirebase(Tables.authors);
                      } else if (status ==
                          statusUploadImageAndAddAuthor.failUploadStorage) {
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
