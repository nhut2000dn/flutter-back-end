...
class AddSlideshowPage extends StatefulWidget {
  @override
  _AddSlideshowPageState createState() => _AddSlideshowPageState();
}

enum statusUploadImageAndAddSlideshow {
  sucess,
  fail,
  failUploadStorage,
  failAddFirestore
...
class _AddSlideshowPageState extends State<AddSlideshowPage> {
  ...
  Future<statusUploadImageAndAddSlideshow> uploadImageAndAddGenreToFirebase(
      String title, int index, String novelId) async {
    statusUploadImageAndAddSlideshow status =
        statusUploadImageAndAddSlideshow.fail;
    if (_imageFile!.path != '') {
      ...
      await Future.value(uploadTask)
          .then((value) async => {
                ...
                if (await addSlideshowToFirebae(
                    title, await value.ref.getDownloadURL(), index, novelId))
                  {status = statusUploadImageAndAddSlideshow.sucess}
                else
                  {status = statusUploadImageAndAddSlideshow.failAddFirestore}
              })
          .onError((error, stackTrace) => {
                debugPrint("Upload file path error ${error.toString()} "),
                status = statusUploadImageAndAddSlideshow.failUploadStorage
              });
    } else {
      if (await addSlideshowToFirebae(title, '', index, novelId)) {
        status = statusUploadImageAndAddSlideshow.sucess;
      } else {
        status = statusUploadImageAndAddSlideshow.failAddFirestore;
      }
                ...
                child: Container(
                  ...
                  child: FlatButton(
                    onPressed: () async {
                      ...
                      statusUploadImageAndAddSlideshow status =
                          await uploadImageAndAddGenreToFirebase(
                        _titleController.text,
                        int.parse(_indexController.text),
                        novelCurrent!.id,
                      );
                      if (status == statusUploadImageAndAddSlideshow.sucess) {
                        locator<NavigationService>().goBack();
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content: Text("Add Slideshow Succesful!")));
                        tablesProvider.refreshFromFirebase(Tables.slideshows);
                      } else if (status ==
                          statusUploadImageAndAddSlideshow.failUploadStorage) {
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
